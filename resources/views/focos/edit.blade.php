...
<body class="bg-violet-100 font-sans antialiased flex items-center justify-center min-h-screen">
    <div class="container mx-auto p-4 sm:p-8">
        <div class="bg-violet-50 rounded-lg shadow-xl p-6 sm:p-8 max-w-lg mx-auto">
            <h1 class="text-3xl font-bold text-violet-900 text-center mb-6">Editar Foco</h1>
            <p class="text-violet-700 text-center mb-8">Edita la información del foco.</p>
            ...
            <form action="{{ route('focos.update', $foco->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="tipo" name="tipo" value="{{ old('tipo', $foco->tipo) }}" class="w-full bg-yellow-200 text-violet-900 border border-yellow-300 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-purple-500" required>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select id="aula_id" name="aula_id" class="w-full bg-yellow-200 text-violet-900 border border-yellow-300 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-purple-500" required>
                        ...
                        @foreach($aulas as $aula)
                            <option value="{{ $aula->id }}" @if(old('aula_id', $foco->aula_id) == $aula->id) selected @endif>{{ $aula->nombre }}</option>
                        @endforeach
                    </select>
                </div>
