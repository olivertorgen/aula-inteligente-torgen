...
<body class="bg-violet-100 font-sans antialiased flex items-center justify-center min-h-screen">
    <div class="container mx-auto p-4 sm:p-8">
        <div class="bg-violet-50 rounded-lg shadow-xl p-6 sm:p-8 max-w-lg mx-auto">
            <h1 class="text-3xl font-bold text-violet-900 text-center mb-6">Crear Nueva Aula</h1>

            <form action="{{ route('aulas.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label for="nombre" class="block text-violet-800 text-sm font-semibold mb-2">Nombre del Aula</label>
                    <input type="text" name="nombre" id="nombre" class="w-full px-4 py-2 border border-violet-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-400" required>
                </div>

                <div class="mb-4">
                    <label for="ubicacion" class="block text-violet-800 text-sm font-semibold mb-2">Ubicación</label>
                    <input type="text" name="ubicacion" id="ubicacion" class="w-full px-4 py-2 border border-violet-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-400" required>
                </div>

                <div class="mb-6">
                    <label for="capacidad" class="block text-violet-800 text-sm font-semibold mb-2">Capacidad</label>
                    <input type="number" name="capacidad" id="capacidad" class="w-full px-4 py-2 border border-violet-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-400" required>
                </div>
            ...
            <div class="mt-6 text-center">
                <a href="{{ route('aulas.index') }}" class="text-violet-600 hover:text-violet-900 transition duration-300 ease-in-out font-medium">Volver al listado</a>
            </div>
        </div>
    </div>
</body>
