...
<body class="bg-violet-100 font-sans antialiased flex items-center justify-center min-h-screen">
    <div class="container mx-auto p-4 sm:p-8">
        <div class="bg-violet-50 rounded-lg shadow-xl p-6 sm:p-8 max-w-lg mx-auto">
            <h1 class="text-3xl font-bold text-violet-900 text-center mb-6">Crear Cortina</h1>
            <p class="text-violet-700 text-center mb-8">Completa el formulario para agregar una nueva cortina.</p>
            
            <form action="{{ route('cortinas.store') }}" method="POST">
                @csrf
                <div class="mb-6">
                    <label for="tipo" class="block text-violet-800 font-semibold mb-2">Tipo</label>
                    <input type="text" id="tipo" name="tipo" value="{{ old('tipo') }}" class="w-full bg-yellow-200 text-violet-900 border border-yellow-300 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-purple-500" required>
                </div>

                <div class="mb-6">
                    <label for="color" class="block text-violet-800 font-semibold mb-2">Color</label>
                    <input type="text" id="color" name="color" value="{{ old('color') }}" class="w-full bg-yellow-200 text-violet-900 border border-yellow-300 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-purple-500" required>
                </div>
